...
def softmax_loss_naive(W, X, y, reg):
  ...
  for example in range(num_of_examples):
    loss -= np.log(scores[example][y[example]])
    
    for c in range(num_of_classes):
      if(c == y[example]):
        dW[:, c] += (scores[example][c] - 1)*X[example]
      else:
        dW[:, c] += (scores[example][c])*X[example]
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  mask[range(num_of_examples), y] = 1
  temp_scores = scores - mask
  dW += np.dot(X.T, temp_scores)
